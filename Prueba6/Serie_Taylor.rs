...
fn main(){
    ...
    let senx:f32 = taylor(x, i, n); //resultado de seno de pi/4
    println!("Seno de pi/4 es: {}", senx); //imprimir resultado
}

fn taylor(x:f32, i:f32, n:f32) -> f32{ //funcion de serie de taylor recursiva
    if n<i{
        return taylor(x, i, n+2.0)+((potencia(1.0, x, n)*-1.0)/factorial(n));
    }
    return x;
}
